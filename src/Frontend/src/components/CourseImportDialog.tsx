...
import {
  ...
  type CourseImportFileErrorCode,
  type ImportCourseRow,
...
const fileErrorMessages: Record<CourseImportFileErrorCode, string> = {
...
export function CourseImportDialog({ isOpen, onClose, onImport }: CourseImportDialogProps) {
  ...
  const [rows, setRows] = useState<ImportCourseRow[]>([]);
  ...
  const handleFileChange = async (file?: File) => {
    ...
    setParsing(true);
    try {
      setRows(await parseCourseImportFile(file));
    } catch (error) {
      setParseError(
        error instanceof CourseImportFileError
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Import học phần từ Excel" size="fullscreen">
                    ...
                    <tbody>
                      {rows.slice(0, 8).map((row) => (
                        <tr key={row.rowNumber}>
                          ...
                          <td>
                            {row.courseName || (
                              <span className="admin-import-invalid">Thiếu tên</span>
                            )}
                          </td>
                          <td>{row.credits || '—'}</td>
          ...
          {result ? (
            ...
            <>
              ...
              >
                ...
                {importing ? 'Đang lưu...' : `Import ${rows.length || ''} học phần`}
              </button>
            </>
          )}
        </div>
